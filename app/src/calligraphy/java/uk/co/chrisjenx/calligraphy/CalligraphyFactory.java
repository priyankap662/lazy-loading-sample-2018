...
public class CalligraphyFactory {
    ...
    private static final String NAVIGATION_VIEW_ITEM = "design_menu_item_text";

    private String navigationFontPath;
    ...
    protected static int[] getStyleForTextView(TextView view) {
        ...
        } else if (isNavigationViewItemText(view)) {
    ...
    protected static boolean isNavigationViewItemText(View view) {
        return matchesResourceIdName(view, NAVIGATION_VIEW_ITEM);
    }
    ...
    protected static boolean matchesResourceIdName(View view, String matches) {
        ...
        if (view.getId() != View.NO_ID && view.getResources() != null) {
            ...
            if (!TextUtils.isEmpty(entryName)) {
                ...
                matched = resourceEntryName.equalsIgnoreCase(matches);
            }
        }
        return matched;
    }
    ...
    void onViewCreatedInternal(View view, final Context context, AttributeSet attrs) {
        ...
        if (view instanceof NavigationView) {
            navigationFontPath = resolveFontPath(context, attrs);
        }

        if (CalligraphyFactory.isNavigationViewItemText(view) && !TextUtils.isEmpty(navigationFontPath)) {
            final Method setTypeface = ReflectionUtils.getMethod(view.getClass(), "setTypeface");
            Typeface typeface = getDefaultTypeface(context, navigationFontPath);
            if (setTypeface != null && typeface != null) {
                ReflectionUtils.invokeMethod(view, setTypeface, typeface);
            }
        }
    ...
    protected String resolveFontPath(Context context, AttributeSet attrs) {
        String fontPath = CalligraphyUtils.pullFontPathFromView(context, attrs, mAttributeId);
        if (TextUtils.isEmpty(fontPath)) {
            fontPath = CalligraphyUtils.pullFontPathFromTextAppearance(context, attrs, mAttributeId);
            if (TextUtils.isEmpty(fontPath)) {
                fontPath = CalligraphyUtils.pullFontPathFromStyle(context, attrs, mAttributeId);
            }
        }
        return fontPath;
    }
